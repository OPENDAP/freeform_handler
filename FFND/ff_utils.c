...
static int update_image_info(DATA_BIN_PTR dbin)
{
    ...
    if (nt_askexist(dbin, FFF_FILE | FFF_HEADER, "file_title")) {
...
See the FreeForm User's Guide for detailed information.\n"
	...
	/*changed from  "if (1 || !newform_log)" to handle both user
	   and logfile cases, Reza URI/DODS */

	if (newform_log)
	    do_log(newform_log, "\r%3d%% processed", percent_done);
	else {
...
static int transmogrify
 (
    ...
    enum {
	NO_REASON = 0, BECAUSE_REC_HEADERS = 1, BECAUSE_QUERY = 2, BECAUSE_CONVERT = 4, BECAUSE_EQN = 8
    } do_newform = NO_REASON;
	...
	while (pinfo) {
		...
		/* gcc suggests 90s arounf && within || but I don't want to
		   messup the code. 8/12/98 jhrg */
		binary_format = ff_copy_format(
		   ((do_newform & BECAUSE_CONVERT) 
		    || ((do_newform & BECAUSE_EQN) && std_args->cv_subset))
		   && IS_DATA(PINFO_FORMAT(pinfo)) 
		   ? PINFO_MATE_FORMAT(pinfo) : PINFO_FORMAT(pinfo));
#if 0
		/* Here is the original code 8/12/98 jhrg */
		binary_format = ff_copy_format(
		   ((do_newform & BECAUSE_CONVERT) 
		    || (do_newform & BECAUSE_EQN)
		    && std_args->cv_subset) 
    ...
    *tmpfiles = NULL;
	...
	if (IS_BINARY(binary_format)) {
	    ...
	    binary_format = ff_afm2bfm(binary_format);
	    if (!binary_format)
...
static int iccmp(CHAR_ARRAY_TYPE * n1, CHAR_ARRAY_TYPE * n2)
{				/* Comparison function for insert to use */
    return (strcmp(n1->str, n2->str));
}

static int icmp(BIN_ARRAY_TYPE * n1, BIN_ARRAY_TYPE * n2)
{				/* Comparison function for insert to use */
...
#ifdef TIMER
	/* Put the ifdef TIMER stuff,  Reza, URI/DODS */
