...
double ee_evaluate_equation(EQUATION_INFO_PTR einfo, int *error)
{
	...
	if (einfo->equation[pos] < 'a') {
		...
		case '$':	/* Substring */
			...
			einfo->eqn_vars[einfo->equation[++pos]] = FALSE;
		    continue;
		case '%':	/* modulus */
		    x = einfo->eqn_vars[einfo->equation[++pos]];
		    if ((y = einfo->eqn_vars[einfo->equation[++pos]]) == 0)
		...
		case 'H':	/* rnd */
		    x = einfo->eqn_vars[einfo->equation[++pos]];
		    if (x == 0)
			einfo->eqn_vars[einfo->equation[++pos]] = 0;
		    else
			einfo->eqn_vars[einfo->equation[++pos]] = floor(x + 0.50);
		    continue;
		case 'I':	/* sqr */
		    x = einfo->eqn_vars[einfo->equation[++pos]];
		...
		case 'X':	/* sign */
			...
			einfo->eqn_vars[einfo->equation[++pos]] = x / fabs(x);
		    continue;
		}
	    }
	    switch (einfo->equation[pos]) {
...
/*** now, do simple operations in order of importance ***/
    if (!ee_replace_op(eqn, "^", '^', 0, ' ', einfo, error))
	return (0);
/*** exponentiation ***/
...
/*** multiplication ***/
	    ...
	    if (!(position_two = ee_get_prev_num(position, error)))
		return (0);
	    ...
	    if (ee_check_for_char(x, y, einfo, error))
		return (0);
...
/*** subtraction ***/
    ...
    if (!ee_replace_op(eqn, "$>", '$', 42, '$', einfo, error))
	return (0);		/* substring */
    if (!ee_replace_op(eqn, "<=", 'Q', 1, 'S', einfo, error))
	return (0);		/* less than or equal to */
    if (!ee_replace_op(eqn, ">=", 'R', 1, 'T', einfo, error))
	return (0);		/* greater than or equal to */
    if (!ee_replace_op(eqn, "!=", 'Y', 1, 'Z', einfo, error))
	return (0);		/* not equal to */
    if (!ee_replace_op(eqn, "=", 'L', 1, 'U', einfo, error))
	return (0);		/* equal to */
    if (!ee_replace_op(eqn, "<", 'M', 1, 'V', einfo, error))
	return (0);		/* less than */
    if (!ee_replace_op(eqn, ">", 'N', 1, 'W', einfo, error))
	return (0);		/* greater than */
    if (!ee_replace_op(eqn, "&", 'O', 0, ' ', einfo, error))
	return (0);		/* and */
    if (!ee_replace_op(eqn, "x|", '|', 0, ' ', einfo, error))
	return (0);		/* exclusive or */
    if (!ee_replace_op(eqn, "|", 'P', 0, ' ', einfo, error))
	return (0);		/* or */
...
int ee_replace_op(char *eqn, char *operator, char repwith, char char_allowed, char crepwith, EQUATION_INFO_PTR einfo, int *error)
{
	...
	if (char_allowed) {	/* character data types allowed */
		...
		v = einfo->variable_type[x];
	    if (y < (int) einfo->result)
		w = einfo->variable_type[y];
	    if (v == w) {
		if (v == EE_VAR_TYPE_CHAR)
		    einfo->equation[z] = crepwith;
...
char *ee_get_prev_num(char *eqn, int *error)
{
	...
	*(error) = EE_ERR_EQN_BAD;
    ...
    /* Added as per gcc warning: control reaches end of non-void function.
       8/12/98 jhrg */
    return (NULL);
}
...
EQUATION_INFO_PTR ee_clean_up_equation(char *eqn, int *error)
{
	...
	*(error) = EE_ERR_MEM_LACK;
    ...
    position = strchr(eqn, '\"');
    while (position) {
	for (i = 1; i < (signed int) strlen(position); i++)
	    if (position[i] == '\"')
		break;
	j = atoi(position + 1);
	k = j;
	...
	*error = ee_replace(position, ++i, j);
    ...
    for (i = 0; i < (signed int) strlen(eqn); i++) {
	if (eqn[i] == '[')
	    inside_variable_name = 1;
	if (eqn[i] == ']')
	    inside_variable_name = 0;
	if (((isdigit((int)eqn[i]) != 0) || (eqn[i] == '.') 
	     || (eqn[i] == '~')) && !inside_variable_name) {
	    if (eqn[i] == '~')
		eqn[i] = '-';
	    j = i;
