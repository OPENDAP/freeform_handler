...
int mn_proc_section_free(MENU_SECTION_PTR menu_sec)
{
    ...
    if (menu_sec->help_title)
	fRee(menu_sec->help_title);
    if (menu_sec->help_sec)
	fRee(menu_sec->help_sec);
...
int mn_selection_free(MENU_SELECTION_PTR selection)
{
	...
	fRee(selection->additional_info);
    if (selection->user_display)
	fRee(selection->user_display);
    if (selection->next_event)
	fRee(selection->next_event);
    if (selection->help_title)
	fRee(selection->help_title);
    if (selection->help_sec)
	fRee(selection->help_sec);
