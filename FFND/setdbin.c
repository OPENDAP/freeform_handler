 ...
 (
	...
	default:
		...
		if (status == INITIAL || status == SIGN || status == INTEGER)
		    ...
		    return (FFV_TEXT);
	    } else if (isspace((int)header[start - 1])) {	/* space */
		if (status == INITIAL);
...
static void set_var_typeprec
 (
	...
	if (v_shift + (size_t) var->start_pos < (size_t) var->end_pos) {	/* count trailing characters after decimal point */
 ...
 *
 * DESCRIPTION:
 * Parse header_text given either 1) namevalue_delim (the delimeter between
 * a parameter value and its name) and valuename_delim (the delimeter
 ...
 *
 * Create variables for both parameter name (and its delimeter) and parameter
...
static int create_fully_dynamic_header_format
 (
	...
	/* Find parameter name */
	if (name_value_delim)
	    end = strcspn(header_text + start, name_value_delim) + start - 1;	/* replace with multi-char delimeter search */
	else if (distance)
	    end = start + distance;
	else {
	    error = err_push(ERR_GENERAL, "distance or a name-value delimeter must be defined");
	    break;
	}
	...
	if (value_name_delim)
	    end = strcspn(header_text + start, value_name_delim) + start - 1;	/* replace with multi-char delimeter search */
	else {
	    error = err_push(ERR_GENERAL, "value-name delimeter must be defined");
	    break;
	}
 ...
 (
	...
	*hvar = ff_create_variable(item_name ? item_name : "");
	...
	if (pname) {
	    ...
	    if (name_dist == 0 && name_terminator_str[0] == STR_END) {	/* item_names are not in header */
		pname = header;
	    } else {
	...
	if (start_pos == 0 && name_terminator_str[0] != STR_END) {	/* string begins with name_terminator */
	    start_pos = 1;

	    error = new_name_string__("no name", &(*hvar)->name);
	    if (error)
		return (error);
	} else if (!offset_found || name_terminator_str[0] == STR_END) {	/* string does not contain name_terminator, or name_terminator is NULL */
	    start_pos = 0;
    ...
    if (end_pos == 0) {		/* string begins with value_terminator, or at end of string */
    } else if (!offset_found) {	/* string does not contain value_terminator, or value_terminator is NULL */
    } else {
...
static int header_to_format
 (
	...
	while (var != NULL) {
	    ...
	    if (!strcmp(name_terminator, "") && name_dist == 0) {	/* item_names are not in header */
...
{
    enum {
	GBEOLS_EOL_LF = 10, GBEOLS_EOL_CR = 13
    };
    char *c;
...
static int finishup_input_header
 (
    ...
    if (IS_VARIED_TYPE(PINFO_TYPE(pinfo))) {	/* get the delimiter 1, delimiter 2 and/or distance */
...
static int find_format_files
 (
    ...
    char *format_files[NUM_FMT_FILES] =
    {NULL, NULL};
    int num_found;
...
static int 
make_format_data (char *dbin_file_name,
...
static int set_array_mappings
 (
	...
	if (IS_HEADER(PINFO_FORMAT(pinfo))) {
		...
		if (PINFO_IS_FILE(pinfo) && IS_INPUT_TYPE(PINFO_TYPE(pinfo))) {
...
static long length_in_file(PROCESS_INFO_PTR pinfo)
{
    ...
    /* return added as per gcc warning. 8/12/98 jhrg */
    return (0);
}

int 
update_following_offsets_or_size (PROCESS_INFO_PTR updater,
