...
{
    short days_per_month[13] =
    {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365};
    short days_per_leap_month[13] =
...
#endif
    static double century_and_year, century, year, month, day, hour,
     minute, second;
    static long int_century_and_year, int_century, int_year, int_month,
     int_day, int_hour, int_minute, int_second;
    int error;
    int variable_length;
...
int cv_date_string(VARIABLE_PTR out_var, double *output, FORMAT_PTR input_format, FF_DATA_BUFFER input_buffer)
{
    ...
    if (string_number < DATE_NAMES_COUNT) {	/* Input variable is data string */
	...
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, sec_str, input_format->type);

	in_var = ff_find_variable("minute", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, min_str, input_format->type);

	in_var = ff_find_variable("hour", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, hour_str, input_format->type);

	in_var = ff_find_variable("day", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, day_str, input_format->type);

	in_var = ff_find_variable("month", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, month_str, input_format->type);

	in_var = ff_find_variable("year", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, year_str, input_format->type);

	in_var = ff_find_variable("century", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, century_str, input_format->type);
 ...
 */
    ...
    if (!last_underscore)
	return (0);		/* Invalid output variable name */
    ++last_underscore;
 ...
 */
	...
	*last_underscore = STR_END;
	...
	var_source = ff_find_variable(v_name, input_format);
	if (var_source) {	/*  value in meters found */
 ...
 */
	...
	*last_underscore = STR_END;
	...
	var_source = ff_find_variable(v_name, input_format);
	if (var_source) {	/*  value in feet found */
	    /* Initialize the return value to zero */
	    double_ptr = &double_value;
	    *converted_value = 0;
 ...
 */
    ...
    var_source = ff_find_variable(v_name, input_format);
    if (var_source) {		/* absolute value found */
...
{				/* Input Buffer */
    ...
    *converted_value = 0;	/* Initialize the return value to zero */
	...
	if (double_value < 180.0)
	    *converted_value += double_value;
	else
	    *converted_value = double_value - 360.0;
	return (1);
    }
    ...
    if (double_value > .000000000000001)
	*converted_value += double_value;
    else
	*converted_value = 360 + double_value;
    return (1);
}
...
{				/* Input Buffer */
	...
	*converted_value = 60.0 * fmod(*converted_value, 1.0);
	...
	if (var->precision) {
	    ...
	    *converted_value = (double) ((int) *converted_value);

	if (*converted_value && negate_value)
	    *converted_value *= -1.0;
...
int cv_mag2long
 (
    ...
    if (!mag_var)
	mag_var = ff_find_variable("mb", input_format);
    ...
    if (!mag_var)
	mag_var = ff_find_variable("ms", input_format);
    if (!mag_var)
	mag_var = ff_find_variable("ms1", input_format);
    ...
    if (!mag_var)
	mag_var = ff_find_variable("magnitude_ml", input_format);
    if (!mag_var)
	mag_var = ff_find_variable("magnitude_local", input_format);
    if (!mag_var)
	mag_var = ff_find_variable("ml", input_format);
    if (!mag_var)
	mag_var = ff_find_variable("ms2", input_format);
...
int 
cv_noaa_eq(VARIABLE_PTR var, /* Description of Desired variable */
	   double *converted_value,	/* Pointer to the Converted value */
	   FORMAT_PTR input_format,	/* Input Format Description */
	   FF_DATA_BUFFER input_buffer) /* Input Buffer */
{
...
/*      char _temp[100]; */
    ...
    /* Initialize the variable destination (a double) to blanks */
    for (i = 8, dummy_ch_ptr = (char *) dummy; i; i--, ++dummy_ch_ptr)
	*dummy_ch_ptr = ' ';
	...
	if (ch_ptr) {
	    if (*(ch_ptr - 5) == ' ')
		*(dummy) = strtod(ch_ptr - 4, NULL);
	    else
		*(dummy) = strtod(ch_ptr - 5, NULL);
	    return (1);
	}
...
int cv_time_string(VARIABLE_PTR out_var, double *output, FORMAT_PTR input_format, FF_DATA_BUFFER input_buffer)
{
    ...
    *second = STR_END;
	...
	in_var = ff_find_variable("hour", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, hour, input_format->type);

	in_var = ff_find_variable("minute", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, minute, input_format->type);

	in_var = ff_find_variable("second", input_format);
	if (in_var)
	    ff_get_string(in_var, input_buffer + in_var->start_pos - 1, second, input_format->type);
    }
    if (!in_var)
	return (0);
...
int cv_geo44tim
 (
    ...
    if (strncmp(data_source, "1000", 4)) {	/* Not header */
