...
static int 
get_format_type_and_title (FORMAT_PTR format,
...
static int display_record_variable_list
 (
	...
	sprintf(bufsize->buffer + bufsize->bytes_used, "%*s %*d %*d ",
		vdf.var_fw, IS_EOL(var) ? "EOL" : var->name,
		(int)vdf.start_pos_fw, (int)(var->start_pos + array_offset),
		(int)vdf.end_pos_fw, (int)(var->end_pos + array_offset));
	bufsize->bytes_used += strlen(bufsize->buffer + bufsize->bytes_used);
...
static int 
dbask_format_list_description (PROCESS_INFO_LIST plist,
			       FF_BUFSIZE_PTR bufsize) 
{
#if 0
    int format_num = 0;
...
static int 
dbask_format_list_description_to_user (PROCESS_INFO_LIST plist,
				       FF_BUFSIZE_PTR bufsize) 
{
#if 0
    int format_num = 0;
