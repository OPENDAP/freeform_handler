...
static int convert_formats
 (
	...
	} else if (!(PINFO_NEW_RECORD(output_pinfo) || (IS_FILE_HEADER(PINFO_FORMAT(output_pinfo)) && PINFO_ARRAY_DONE(output_pinfo)))) {	/* Hack Hack Hack Hack Hack Hack Hack Hack Hack Hack Hack Hack Hack Hack */
...
static BOOLEAN 
need_to_read_next_record_header (PROCESS_INFO_PTR pinfo, DATA_BIN_PTR dbin) 
{
    FF_VALIDATE(pinfo);
    FF_VALIDATE(dbin);
    ...
    /* gcc warns control reaches end of non-void function. Added
       return(FALSE). 8/12/98 jhrg */
    return (FALSE);
}
